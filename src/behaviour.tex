...
\begin{definition}
	A \textbf{theory of behaviour} or just \textbf{behaviour} for a systems theory $\Sys$ valued in the Cartesian category $\cat C$ is a map of system theories $\systh B:\Sys \to \BehSys(\cat C)$:
	\begin{equation}
...
While $\doc{Beh}$ is very often a general enough doctrine of behavioural systems sometimes it comes short of its role.
...
In particular, as observed in~\cref{ex:behav-systems}, behavioural theories have a logical flavour: their notion of system is that of a display map in a categorical model of dependent type theory. In other words, $\BehSys(\cat C)$ is strongly related to the codomain fibration $\cod : \cat C^\downarrow \to \cat C$.
...
\begin{example}
	...
	The transformation $\Hom^h$ at a pointed theory $(\Sys : \Processes^\top \to \dblCat, \sys B : \Sys(I))$ is the map of system theories $\Processes^h(\sys B, -) : \Sys \to \BehSys(\Set)$ defined as follows.
