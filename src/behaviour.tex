...
\label{rmk:states-to-reach}
	We can already see an example of \emph{map} of behaviours. In fact consider the behaviour of states $\states : \Trans \to \BSys$ and the behaviour of reachability $R : \Trans \to \BSys$:
	\begin{equation}
		...
		\begin{tikzcd}[ampersand replacement=\&]
			...
			\arrow[""{name=1, anchor=center, inner sep=0}, "{\ R}"{pos=0.42}, shift left=4, curve={height=-12pt}, shorten <=25pt, shorten >=25pt, Rightarrow, from=1-2, to=3-2]
	...
	where
	\begin{eqalign}
		\varepsilon^\flat_\Sigma(\delta,\zeta) : \Trans(\Sigma)(\delta,\zeta) &\longto \Set/\varepsilon_\Sigma(\states_\Sigma(\delta), R_\Sigma(\zeta))\\
		...
		\begin{tikzcd}[ampersand replacement=\&,sep=scriptsize]
			{\states_\Sigma(\delta)} \& {R_\Sigma(\delta,\zeta)} \\
			1 \& {\Sigma^*}
...
\begin{remark}
	Notice the observation that~\eqref{eq:fsm-lang} is a sum of corepresentables was already valid for the reachability behaviour $\systh R$ in~\cref{ex:reachability}.
	...
	In this light, the map described in~\cref{rmk:states-to-reach} is simply the inclusion of $\states = \Trans(\sys{Fin\, 0}, -)$ into the sum $\systh R = \sum_{n \geq 0} \Trans(\sys {Fin\, n}, -)$.
\end{remark}
