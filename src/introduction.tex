...
Categorical approaches to general or particular systems theory have been around for a long time.
Here we try to give a short chronology, and we will gather examples as we come across them.
However, the reader should not expect a complete history of the field from this section, but rather a list of pointers to the literature.
The main aim of the following story is to acknowledgme the shoulders of the giants we stand on, and show how the ideas converging in DCST often arisen somewhere else first.

The earliest example of categorical systems theory is probably due to Rosen~\cite{rosen1978fundamentals}, which deals with systems from a behavioural point of view.
The work pioneers the idea of gathering systems in categories and considering their observable behaviour as input-output relations.

In the 70s, lots of automata theory was also interpreted categorically, generalizing it considerably, in the works of Arbib and Manes \cite{arbibRelevanceAbstractAlgebra1969,arbibMachinesCategoryExpository1974,arbibFoundationsSystemTheory1974,arbibAdjointMachinesStatebehavior1975} and Goguen \cite{goguenRealizationUniversal1972,goguenMinimalRealizationMachines1972,goguenCategoricalFoundationsGeneral1973}.
...
Notably, Courser has been the first (to our knowledge) to explicitly recognize double categories as instrumental to the compositional study of open systems, see \cite{courserOpenSystemsDouble2020}.

It is Spivak \cite{spivak2013operad} who first introduced the operadic view of systems, see also Libkind's \cite{libkind2021operadic} for a more mature take.
...
However, it has a large and well-developed literature covering an immense breadth of topics, and delving deeply in many applications such as model checking and semantics of modal logic \cite{mossCoalgebraicLogic1999}.
...
The cosmogony of categorical systems theory can be organized in three levels of decreasing abstraction.
